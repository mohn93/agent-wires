...
List<Tool> perceptionTools(AppSession session, SemanticMap map) => [
      Tool(
        ...
        description:
            ...
            '(button | textfield | tappable | ...), a human `label`, '
            'on-screen `bounds`, and — for stateful widgets — a `state` '
            'field (`"on"`/`"off"` for Switch, `"checked"`/`"unchecked"` for '
            'Checkbox, slider value for Slider, etc.). Pass those '
            '`element_id`s to tap, enter_text, '
