...
import 'package:agent_wires_mcp/src/version.dart';
...
Future<void> _runReview(List<String> args) async {
  ...
  if (parsed['help'] as bool) {
    stdout.writeln('agent_wires_mcp review — open the QA review dashboard\n');
...
/// `agent_wires_mcp run [-d <device>] [--project <flutter app dir>]`
///
...
Future<void> _runServe(List<String> args) async {
  ...
  final parsed = parser.parse(args);
  if (parsed['help'] as bool) {
    stdout.writeln('agent_wires_mcp — MCP server for Flutter QA agents\n');
  ...
  if (attach == null) {
    ...
    stderr.writeln('(prefer `agent_wires_mcp run` so the URI is auto-discovered)');
    exit(64);
  }
