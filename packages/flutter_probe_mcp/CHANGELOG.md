...
- `flutter_probe_mcp run` — boots `flutter run --machine`, auto-discovers
  the VM service URI, and serves MCP over stdio in one process.
  Forwards `--flavor`, `-t/--target`, `--dart-define`.
- `flutter_probe_mcp serve --attach <ws-uri>` — attaches to an already-
  running app.
- `flutter_probe_mcp review` — local human-curation dashboard at
...
- Requires the target app to have
  [`flutter_probe`](https://pub.dev/packages/flutter_probe)
  installed.
