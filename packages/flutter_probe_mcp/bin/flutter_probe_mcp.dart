...
import 'package:flutter_probe_mcp/src/dashboard/server.dart';
import 'package:flutter_probe_mcp/src/map/semantic_map.dart';
import 'package:flutter_probe_mcp/src/mcp/protocol.dart';
import 'package:flutter_probe_mcp/src/mcp/transport.dart';
...
import 'package:flutter_probe_mcp/src/tools/logs_tools.dart';
import 'package:flutter_probe_mcp/src/tools/memory_tools.dart';
import 'package:flutter_probe_mcp/src/tools/perception.dart';
import 'package:flutter_probe_mcp/src/tools/sync_tools.dart';
import 'package:flutter_probe_mcp/src/version.dart';
...
Future<void> _runReview(List<String> args) async {
  ...
  if (parsed['help'] as bool) {
    stdout.writeln('flutter_probe_mcp review — open the QA review dashboard\n');
...
/// `flutter_probe_mcp run [-d <device>] [--project <flutter app dir>]`
///
...
Future<void> _runRun(List<String> args) async {
  ...
  stderr.writeln('flutter_probe_mcp: booting Flutter app in $project ...');
  await runner.start();
  stderr.writeln('flutter_probe_mcp: VM service @ ${runner.vmServiceUri}');

  final map = SemanticMap(projectRoot: mapRoot);
  await map.load();

  final vm = await VmClient.connect(runner.vmServiceUri);
  stderr.writeln('flutter_probe_mcp: attached to QA isolate, serving MCP.');
...
Future<void> _runServe(List<String> args) async {
  ...
  final parsed = parser.parse(args);
  if (parsed['help'] as bool) {
    stdout.writeln('flutter_probe_mcp — MCP server for Flutter QA agents\n');
  ...
  if (attach == null) {
    ...
    stderr.writeln('(prefer `flutter_probe_mcp run` so the URI is auto-discovered)');
    exit(64);
  }
