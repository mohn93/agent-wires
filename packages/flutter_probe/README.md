...
[![pub package](https://img.shields.io/pub/v/flutter_probe.svg)](https://pub.dev/packages/flutter_probe)

Runtime probe that exposes a Flutter app's widget tree to QA agents over the
Dart VM service. Pairs with
[`flutter_probe_mcp`](https://pub.dev/packages/flutter_probe_mcp) — together they
...
void main() {
  FlutterProbe.install();          // ① register ext.qa.* on the VM service
  runApp(const MyApp());
}
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      navigatorObservers: [
        FlutterProbe.routeTracker.createObserver(),   // ② track active route
      ],
      home: const HomePage(),
    );
  }
